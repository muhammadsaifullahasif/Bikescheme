...
if(isset($_GET['aid'])) {
	...
	$query = mysqli_query($conn, "SELECT * FROM users WHERE role='2' && agent_id='$aid' && delete_status='1'");
    ...
    if(mysqli_num_rows($q_query) > 0) {
        ...
        $query = mysqli_query($conn, "SELECT * FROM users WHERE id='$user_id' && role='2' delete_status='1'");
    }
    $printUrl = 'print-users.php?q='.$q;
} else if(isset($_GET['status'])) {
    $status = mysqli_real_escape_string($conn, $_GET['status']);
    if($status == 'active') {
        $query = "SELECT * FROM users WHERE role='2' && active_status='1' && delete_status='1'";
        $run = mysqli_query($conn, $query);
    } else if($status == 'inactive' || $status != 'active') {
        $query = "SELECT * FROM users WHERE role='2' && active_status='0' && delete_status='1'";
        $run = mysqli_query($conn, $query);
    }
    $printUrl = 'print-users.php?status='.$status;
} else {
    $query = "SELECT * FROM users WHERE role='2' && delete_status='1'";
					...
					<table class="mb-3 table table-sm table-striped table-hover table-responsive-md" id="user_print">
						<thead class="thead-dark">
							<tr>
								...
								<th class='d-print-none'>Enroll Date</th>
						...
						<tbody>
							...
							<tr>
								...
								<td class='d-print-none'><?php echo date("d-m-Y", strtotime($result['date_created'])); ?></td>
								<td class='d-print-none'>
									<div class="btn-group">
										...
										<button data-did="<?php echo $id; ?>" class="btn btn-danger restore-user">Restore User</button>
									</div>
								</td>
							</tr>
							<?php
	...
	<script type="text/javascript">
		$(document).ready(function(){
			$(document).on('click', '.restore-user', function(){
				if(confirm('Are you sure to restore user?')) {
					var did = $(this).data('did');
					$.ajax({
						url: 'ajax/user-restore.php',
						type: 'POST',
						data: {id:did},
