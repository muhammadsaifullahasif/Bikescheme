		...
		<div class="container-fluid">
			<div class="card mb-3">
				<div class="card-header bg-dark text-white">
					<h5 class="card-title d-inline">Add Images</h5>
					...
					<div class="mb-2">
						...
						if(isset($_POST['submit'])) {
							if($_FILES['image_file']['name'] != '') {
								...
								$time_created = time();
								$query = mysqli_query($conn, "INSERT INTO images(id, scheme_id, title, time_created) VALUES('$id', '$scheme_id', '$image_name', '$time_created')") or die(mysqli_error($conn));
								$media_query = mysqli_query($conn, "INSERT INTO media(scheme_id, for_id, media_name, media_path, media_url, media_alt, media_type, media_size, media_for, time_created) VALUES('$scheme_id', '$id', '$media_name', '$link', '$image_link', '$image_alt', '$media_type', '$media_size', '4', '$time_created')") or die(mysqli_error($conn));
								if($query && $media_query) {
									echo "<div class='alert alert-success'><a class='close' href='#' data-dismiss='alert'>&times;</a>Image is Successfully Added</div>";
								} else {
									echo mysqli_error($conn);
									echo "<div class='alert alert-danger'><a class='close' href='#' data-dismiss='alert'>&times;</a>Image is not Added Please Try Again</div>";
								}
							} else {
								echo "<div class='alert alert-danger'><a class='close' href='#' data-dismiss='alert'>&times;</a>Please Select An Image</div>";
							}
						}
