...
if(isset($_GET['aid'])) {
	...
	$query = mysqli_query($conn, "SELECT c.id, c.user_id, u.scheme_id FROM clearance AS c INNER JOIN users AS u ON c.user_id=u.id WHERE u.role='2' && u.agent_id='$aid' && u.delete_status='0'");
    ...
    if(mysqli_num_rows($q_query) > 0) {
        ...
        $query = mysqli_query($conn, "SELECT c.id, c.user_id, u.scheme_id FROM clearance AS c INNER JOIN users AS u ON c.user_id=u.id WHERE u.id='$user_id' && u.role='2' && u.delete_status='0'");
    }
    $printUrl = 'print-users.php?q='.$q;
} else if(isset($_GET['status'])) {
    $status = mysqli_real_escape_string($conn, $_GET['status']);
    if($status == 'active') {
        $query = "SELECT c.id, c.user_id, u.scheme_id FROM clearance AS c INNER users AS u ON c.user_id=u.id WHERE u.role='2' && u.active_status='1' && u.delete_status='0'";
        $run = mysqli_query($conn, $query);
    } else if($status == 'inactive' || $status != 'active') {
        $query = "SELECT c.id, c.user_id, u.scheme_id FROM clearance AS c INNER JOIN users AS u ON c.user_id=u.id WHERE u.role='2' && u.active_status='0' && u.delete_status='0'";
        $run = mysqli_query($conn, $query);
    }
    $printUrl = 'print-users.php?status='.$status;
} else {
    $query = "SELECT u.id, c.user_id, u.scheme_id FROM clearance AS c INNER JOIN users AS u ON c.user_id=u.id WHERE u.role='2' && u.delete_status='0'";
					    ...
					    <table class="mb-3 table table-sm table-striped table-hover table-responsive-md" id="user_print">
    						<thead class="thead-dark">
    							<tr>
    								...
    								<th class='d-print-none'>Total Paid Amount</th>
    						...
    						<tbody>
    							...
    							if(mysqli_num_rows($run) > 0) {
    								...
    								while($result = mysqli_fetch_assoc($run)) {
                                        ...
                                        $clearance_query = mysqli_query($conn, "SELECT * FROM clearance WHERE user_id='$id'");
                                        $clearance_result = mysqli_fetch_assoc($clearance_query);
    							?>
    							<tr>
    								<td><?php echo $i; ?></td>
    								...
    								<td class='d-print-none'><b>Rs: <?php echo $clearance_result['total_paid_amount']; ?></b></td>
    								<td class='d-print-none'>
    									<div class="btn-group">
    										...
    										if(mysqli_num_rows($clearance_query) == 1) {
